...
class DeepLatentReconstruction():
    def __init__(self, autoencoder: variational_auto_encoder.VariationalAutoEncoder) -> None:
    ...
    def pet_step(self, current, pet_decoded, mu):
        ...
        square_root_term = ( (pet_decoded - mu - (sensitivity/self.rho_pet))**2
                             + (4 * x_em * sensitivity)/self.rho_pet)
        new_xpet = 0.5 * (pet_decoded - mu - (sensitivity/self.rho_pet) + np.sqrt(square_root_term))
    ...
    def pet_reconstruction_step(self, x, mu, z):
        ...
        new_x = np.concatenate((np.expand_dims(new_x_pet, axis=-1), x[:,:,1:]), axis=-1)
        ...
        #RESIDUALS
        norm_primal, norm_dual = self.compute_residuals(new_x[:,:,0], new_mu[:,:,0], new_decoded[:,:,0], decoded[:,:,0])
    ...
    def pet_reconstruction(self, x_pet0, ref_mr, y_pet, projector_id, step_size):
        #Initializations
        nb_steps = 40
        ...
        *_, z = self.autoencoder.encoder(np.expand_dims(x0_normalized, axis=0))
        ...
        x = np.concatenate(
            (
                np.expand_dims(x_pet0, axis=-1),
                np.expand_dims(ref_mr, axis=-1)
            ),
            axis=-1
        )
        mu = np.zeros_like(x)
    ...
    def pet_reconstruction_metrics(self, x_pet0, ref_pet, ref_mr, y_pet, projector_id, step_size):
        #Initializations
        nb_steps = 40
        self.step_size = step_size
        self.y_pet = y_pet
        ...
        *_, z = self.autoencoder.encoder(np.expand_dims(x0_normalized, axis=0))
        ...
        for _ in range(nb_steps):
            ...
            points_pet["ssim"].append(ssim(x_pet, ref_pet,
                                        data_range=x_pet.max()))
